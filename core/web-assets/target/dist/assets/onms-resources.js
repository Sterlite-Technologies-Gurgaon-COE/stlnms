...
(0, _filter.default)(_context = angular.module('onms-resources', ['onms.http', 'onms.default.apps', 'ui.bootstrap', 'angular-growl']).config(['growlProvider', function (growlProvider) {
  ...
  $scope.init = function (nodeCriteria, reports, endUrl) {
    ...
    $http.get('rest/resources/fornode/' + nodeCriteria).then(function succeeded(response) {
      ...
      }), 'typeLabel'); // Perform a shallow copy of the resource map - the resources may be updated asynchronously
  ...
  $scope.doGraph = function (selected) {
    ...
    if (selected.length > 0) {
      $http.post('rest/resources/generateId', selected).success(function (response) {
