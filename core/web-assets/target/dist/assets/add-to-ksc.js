...
var path = '/home/administrator/Desktop/master27/core/web-assets/src/main/assets/js/apps/add-to-ksc/add-to-ksc.html';
...
angular.module('onms-ksc', ['onms.http', 'onms.default.apps', 'ui.bootstrap', 'angular-growl']).config(['growlProvider', function (growlProvider) {
...
["$http", "$q", function ($http, $q) {
  ...
  var graphSearch = {};
  graphSearch.graphs = [];
  graphSearch.noMatchingGraphs = false; // Update search query and broadcast an update to controllers.
  ...
  graphSearch.updateGraphsWithSearchItem = function () {
    $rootScope.$broadcast('handleSearchQuery'); // Check if there is atleast one matching graph else update correponding controller.
  ...
  var graphTitle = $attrs.graphtitle; // Update service with graphname and graphtitle.

  graphSearchFactory.initialize(graphName, graphTitle);
  $scope.enableGraph = true; // Handle search query update and enable graphs with matching search query.

  $scope.$on('handleSearchQuery', function () {
    var searchQuery = graphSearchFactory.searchQuery; // Filter on graphName or graphTitle.
