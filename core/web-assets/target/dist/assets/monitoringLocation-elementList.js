...
var mainTemplate = __webpack_require__(/*! ./main.html */ "./src/main/assets/js/apps/onms-elementList/monitoringLocation-elementList/main.html"); // $filters that can be used to create human-readable versions of filter values
...
.controller('MonitoringLocationsListCtrl', ['$scope', '$location', '$window', '$log', '$filter', 'monitoringLocationFactory', function ($scope, $location, $window, $log, $filter, monitoringLocationFactory) {
  $log.debug('MonitoringLocationsListCtrl initializing...'); // Set the default sort and set it on $scope.$parent.query

  $scope.$parent.defaults.orderBy = 'locationName';
  $scope.$parent.query.orderBy = 'locationName';
  $scope.newItem = {}; // Reload all resources via REST
  ...
  $scope.$parent.update = function (item) {
    ...
    var saveMe = monitoringLocationFactory.get({
      ...
      saveMe.priority = item.priority; // We have to provide the locationName here because it has a dash in its
      // name and we can't use dot notation to refer to it as a default param
      ...
      if (response.status === 404) {
        ...
        saveMe.latitude = item.latitude;
        saveMe.longitude = item.longitude;
        saveMe.priority = item.priority; // Insert the object instead of updating it

        saveMe.$save({}, function () {
          $scope.refresh();
          $scope.newItem = {}; // Return true to indicate successful submission
